...
contract ERC20 is IERC20 {
  ...
  function transferFrom(address sender, address recepient, uint amount) external enoughTokens(sender, amount) {
    _beforeTokenTransfer(sender, recepient, amount);
    require(allowances[sender][recepient] >= amount, "Chek allowance!");
    allowances[sender][recepient] -= amount; //error!
...
contract MShop {
  ...
  event Solid(uint _amount, address indexed _seller);
  ...
  function sell(uint _amountToSell) external {
    ...
    payable(msg.sender).transfer(_amountToSell); //limiGasa 2300

    emit Solid(_amountToSell, msg.sender);
  }
